...
// --- Hidden admin button ---
let holdTimer;
secretBtn.addEventListener("mousedown",()=>{ holdTimer=setTimeout(()=>keypad.classList.remove("hidden"),800); });
secretBtn.addEventListener("mouseup",()=>clearTimeout(holdTimer));
secretBtn.addEventListener("touchstart",()=>{ holdTimer=setTimeout(()=>keypad.classList.remove("hidden"),800); });
...
function handleKey(k){
  if(k==="C"){inputCode="";adminMessage.textContent="";}
  else if(k==="E"){checkCode();}
  else{
...
function showMenu(){
  ...
  inp2.type="number"; inp2.placeholder="Max clicks"; inp2.onchange=()=>maxClicks=parseInt(inp2.value)||maxClicks;
  ctrl.appendChild(inp2);
  let inp3=document.createElement("input");
  inp3.type="number"; inp3.placeholder="Darks per click"; inp3.onchange=()=>darksPerClick=parseInt(inp3.value)||darksPerClick;
